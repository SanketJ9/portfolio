...
export const preLoaderAnim = () => {
  tl.to("body", {
    ...
    .from(".links .item", {
      duration: 0.5,
      opacity: 0,
      // Added safety check for window
      delay: typeof window !== 'undefined' && window.innerWidth < 763 ? -3 : -0.6,
      stagger: {
        amount: 0.5,
      },
      ease: "expo.easeOut",
      onComplete: animateMainShape, // Note: Removed ()
    })
    .from(".main-circle", {
      duration: 1,
      opacity: 0,
      ease: "power3.easeInOut",
      onComplete: animateShapes, // Note: Removed ()
    })
    .from(".shapes .shape", {
      duration: 1,
      opacity: 0,
...
export const boxHover = (e: any) => {
  const tl = gsap.timeline();
  // Added safety check for window
  if (typeof window !== 'undefined' && window.innerWidth >= 986) {
    tl
